...
const expected = "DEPLOY_RAWAJ_SYRIA_PRODUCTION";
const expectedCommitSha = process.env.RAWAJ_EXPECTED_COMMIT_SHA?.trim() ?? "";
const githubSha = process.env.GITHUB_SHA?.trim() ?? "";

if (action !== "deploy") {
  console.error(
    `Blocked ${action}. Syria remote migrations and other production operations require a separate, explicitly approved workflow.`,
  );
  process.exit(1);
}
...
if (
  ...
  console.error(
    "Blocked deploy. Syria production Worker deployment is allowed only in workflow_dispatch.",
  );
  process.exit(1);
}
