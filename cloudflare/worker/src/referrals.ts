...
export async function handleReferrals(request: Request, env: ReferralsEnv): Promise<Response | null> {
...
async function referralSummary(request: Request, env: ReferralsEnv, cors: Headers) {
  ...
  ).bind(auth.userId).all<{ status: string; count: number }>();
  ...
  return json({ data: { referrals, availableRewardCount: rewards.filter((r) => r.status === "available").length, rewards } }, 200, cors);
}
...
async function claimReferral(request: Request, env: ReferralsEnv, cors: Headers) {
  ...
  if (existing) {
    if (nullable(existing.source_listing_id) === sourceListingId && nullable(existing.referred_listing_id) === referredListingId) {
  ...
  const source = await env.DB.prepare(
    `SELECT l.id, l.owner_id, l.status, l.is_demo, p.account_status, u.disabled_at
      ...
      WHERE l.id = ? LIMIT 1`,
  ).bind(sourceListingId).first<Row>();
  if (!source || !nullable(source.owner_id) || nullable(source.owner_id) === auth.userId || bool(source.is_demo) || text(source.status) === "draft" || text(source.account_status) !== "active" || source.disabled_at) {
  ...
  ).bind(referredListingId, auth.userId).first<Row>();
  if (!referred || bool(referred.is_demo) || !["pending_review", "approved"].includes(text(referred.status))) {
  ...
  ).bind(auth.userId, referredListingId).first<{ id: string }>();
  if (previous) return conflict(cors, "Referral rewards are for a user's first submitted real listing.");
  ...
  ).bind(claimId, text(source.owner_id), auth.userId, sourceListingId, referredListingId, timestamp, timestamp, timestamp).run();
  ...
  await audit(env, auth.userId, "referral.claimed", text(stored.id), { sourceListingId, referredListingId, referrerUserId: text(stored.referrer_user_id) });
  return json({ data: mapClaim(stored), idempotent: false }, 201, cors);
}

async function redeemReward(request: Request, env: ReferralsEnv, cors: Headers, rewardIdRaw: string) {
  const auth = await requireMutationAuth(request, asAuthEnv(env), cors);
  if (auth instanceof Response) return auth;
  ...
  ).bind(rewardId, auth.userId).first<Row>();
  if (!current) return notFound(cors, "Referral reward not found.");
  if (text(current.status) === "redeemed") {
    if (nullable(current.listing_id) !== listingId) return conflict(cors, "This referral reward has already been used on another listing.");
    return json({ data: mapReward(current), idempotent: true }, 200, cors);
  }
  if (text(current.status) !== "available") return conflict(cors, "This referral reward is not available.");
  ...
  ).bind(listingId, promotionRequestId, timestamp, timestamp, rewardId, auth.userId).run();
  ...
  if (!stored) return databaseError(cors);
  await audit(env, auth.userId, "referral.reward_redeemed", rewardId, { listingId, promotionRequestId });
  return json({ data: mapReward(stored), idempotent: false }, 200, cors);
}

async function audit(env: ReferralsEnv, actorId: string, action: string, entityId: string, metadata: Row) {
  const result = await env.DB.prepare(
    `INSERT INTO audit_logs (id,actor_id,action,entity_type,entity_id,metadata,created_at) VALUES (?,?,?,'referral',?,?,?)`,
  ).bind(crypto.randomUUID(), actorId, action, entityId, JSON.stringify(metadata), new Date().toISOString()).run();
  if (!result.success) console.error(JSON.stringify({ event: "referral_audit_write_failed", action, entityId, error: result.error }));
}

function mapClaim(row: Row) {
  return { id: text(row.id), referrerUserId: text(row.referrer_user_id), sourceListingId: nullable(row.source_listing_id), referredListingId: nullable(row.referred_listing_id), status: text(row.status, "claimed") };
}
function mapReward(row: Row) {
  return {
    id: text(row.id), claimId: text(row.claim_id), rewardType: text(row.reward_type, "listing_boost_24h"), status: text(row.status, "available"),
    durationHours: num(row.duration_hours) || 24, suggestedListingId: nullable(row.suggested_listing_id), suggestedListingTitle: nullable(row.suggested_listing_title),
    listingId: nullable(row.listing_id), listingTitle: nullable(row.redeemed_listing_title), promotionRequestId: nullable(row.promotion_request_id),
    grantedAt: nullable(row.granted_at), redeemedAt: nullable(row.redeemed_at), createdAt: nullable(row.created_at), updatedAt: nullable(row.updated_at),
  };
}
function redemptionFailure(cors: Headers, error?: string) {
  const value = error ?? "";
  if (value.includes("referral_reward_listing_not_eligible")) return conflict(cors, "Choose an approved, active listing that belongs to you.");
  if (value.includes("referral_reward_listing_already_featured")) return conflict(cors, "This listing already has an active boost.");
  if (value.includes("referral_reward_promotion_pending")) return conflict(cors, "This listing already has a promotion request under review.");
  if (value.includes("referral_reward_missing_listing")) return validation(cors, "A listing is required to redeem this reward.");
...
function nullable(value: unknown): string | null { const valueText = text(value).trim(); return valueText || null; }
