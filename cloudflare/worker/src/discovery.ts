...
async function listingFacets(url: URL, env: DiscoveryEnv, cors: Headers): Promise<Response> {
  const taxonomyNodeIds = unique(
    url.searchParams.getAll("taxonomyNodeIds").flatMap((v) => v.split(",")),
  ).slice(0, 50);
  ...
  const total = await env.DB.prepare(
    `SELECT COUNT(*) AS total FROM listings l WHERE ${where.clauses.join(" AND ")}`,
  )
    .bind(...where.values)
    .first<{ total: number }>();
  ...
  if (schemaTokens.length === 0) {
    return json(
      { data: { taxonomyVersionId: null, totalCount: numberValue(total?.total), facets: [] } },
      200,
      cors,
    );
  }
  ...
  for (const field of fields.results ?? []) {
    ...
    if (["number", "integer", "decimal", "range"].includes(fieldType)) {
      const bounds = await env.DB.prepare(
        ...
        .bind(path, path, ...baseValues, path)
        .first<Row>();
      ...
      const counts = await env.DB.prepare(
        ...
        .bind(path, ...baseValues, path, path)
        .all<Row>();
      ...
      options = (counts.results ?? []).flatMap((row) => {
        ...
        return [
          {
            valueKey,
            labelAr: labels?.labelAr ?? valueKey,
            labelEn: labels?.labelEn ?? null,
...
async function nearbyListings(url: URL, env: DiscoveryEnv, cors: Headers): Promise<Response> {
  ...
  const where = [
    ...
    "n.latitude BETWEEN ? AND ?",
    "n.longitude BETWEEN ? AND ?",
  ];
  const values: Value[] = [
    latitude - latDelta,
    latitude + latDelta,
  ...
  const ranked = (result.results ?? [])
    ...
    .slice(0, limit)
    .map((item) => ({
      listing: mapListing(item.row, url.origin),
...
async function sitemapReferences(env: DiscoveryEnv, cors: Headers): Promise<Response> {
  const [categories, governorates] = await Promise.all([
    env.DB.prepare(
      "SELECT slug FROM categories WHERE is_active = 1 ORDER BY sort_order, slug",
    ).all<{ slug: string }>(),
    env.DB.prepare(
      "SELECT slug FROM governorates WHERE is_active = 1 ORDER BY sort_order, slug",
    ).all<{ slug: string }>(),
  ]);
  if (!categories.success || !governorates.success) return databaseError(cors);
  return json(
    { data: { categories: categories.results ?? [], governorates: governorates.results ?? [] } },
    200,
    cors,
  );
}
...
function publicWhere(url: URL, attributes: Record<string, AttributeFilter>) {
  ...
  if (
    ...
    return { ok: false as const, message: "Invalid price range." };
  if (min !== null) {
...
function applyAttributes(
  ...
  for (const [key, filter] of Object.entries(attributes)) {
    ...
    if (typeof filter === "string") {
      clauses.push("CAST(json_extract(l.details, ?) AS TEXT) = ?");
      values.push(path, filter);
    } else if (typeof filter === "boolean") {
      clauses.push("CAST(json_extract(l.details, ?) AS INTEGER) = ?");
      ...
      if (typeof filter.min === "number") {
        clauses.push("CAST(json_extract(l.details, ?) AS REAL) >= ?");
...
async function taxonomySchemaTokens(env: DiscoveryEnv, ids: string[]): Promise<string[]> {
  ...
  const result = await env.DB.prepare(
    ...
    .bind(...ids)
    .all<{ filter_schema_key: string | null }>();
  if (!result.success) return [];
  return unique(
    (result.results ?? [])
      .flatMap((row) => (row.filter_schema_key ?? "").split(/[,+|]/))
      .map((v) => v.trim())
      .filter(Boolean),
  ).slice(0, 20);
}
...
async function optionLabels(env: DiscoveryEnv, setKey: string | null) {
  ...
  for (const row of result.results ?? [])
    map.set(stringValue(row.value_key), {
...
function decodeAttributes(value: string | null): Record<string, AttributeFilter> {
  ...
  try {
    ...
    return parsed && typeof parsed === "object" && !Array.isArray(parsed)
      ? (parsed as Record<string, AttributeFilter>)
      : {};
  } catch {
    return {};
  }
...
function addEqual(where: string[], values: Value[], column: string, value: string | null) {
  if (value) {
    where.push(`${column} = ?`);
    values.push(value);
  }
...
function clampInteger(value: unknown, fallback: number, min: number, max: number): number {
  const n = Number(value);
  return Math.max(min, Math.min(max, Number.isInteger(n) ? n : fallback));
}
