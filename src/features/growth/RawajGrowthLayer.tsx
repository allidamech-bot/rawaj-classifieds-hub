...
export default function RawajGrowthLayer() {
  ...
  const handleShareCard = async () => {
    ...
    try {
      ...
      setNotice(
        text("تعذر فتح المشاركة الآن.", "Could not open sharing right now."),
      );
    } finally {
      setBusyAction(null);
    }
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      setNotice(
        text("تعذر إنشاء الصورة الآن.", "Could not create the image right now."),
      );
    } finally {
      setBusyAction(null);
    }
        ...
        <div className="sticky top-0 z-10 flex items-start gap-3 border-b border-border/60 bg-background/95 p-4 backdrop-blur sm:p-5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h2 id="rawaj-share-prompt-title" className="mt-0.5 text-xl font-black text-foreground">
              {text(
                "شارك إعلانك من الآن واجذب المهتمين",
            ...
            <>
              ...
              <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <button
                  ...
                  disabled={Boolean(busyAction)}
                  className="min-h-12 rounded-2xl border border-border bg-card px-4 py-3 text-sm font-black"
                >
                  <span className="flex items-center justify-center gap-2">
