...
export async function fetchPublicListingFacets(
  ...
  if (
    (query.taxonomyNodeIds?.length ?? 0) > 250 ||
    Object.keys(query.attributeFilters ?? {}).length > 50
  ) {
  ...
  return result.ok
    ...
    : {
        ok: false,
        error: { code: result.error.code as ClassifiedsErrorCode, message: result.error.message },
      };
}
