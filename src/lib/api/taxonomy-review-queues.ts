...
export async function fetchTaxonomyMappingQueue(
  ...
  const { data, error } = await clientResult.data.rpc("rawaj_admin_fetch_taxonomy_mapping_queue_v1", {
...
export async function applyConfirmedTaxonomyMapping(
  ...
  if (error) return rpcFailure(error, "taxonomy_mapping_apply");
  return { ok: true, data: record(data) };
}
...
export async function fetchVehicleReferenceQueue(
  ...
  const { data, error } = await clientResult.data.rpc("rawaj_admin_fetch_vehicle_reference_queue_v1", {
    p_status: options.status ?? null,
...
export async function reviewVehicleReference(
  ...
  const { data, error } = await clientResult.data.rpc("rawaj_admin_review_vehicle_reference_v1", {
  ...
  if (error) return rpcFailure(error, "vehicle_reference_review");
  return { ok: true, data: record(data) };
}
...
export async function createVehicleReferenceFromQueue(
  ...
  if (!userId) return authenticationFailure();
  if (!input.queueId.trim() || !input.expectedQueueUpdatedAt.trim() || !input.reference.id.trim()) {
  ...
  if (error) return rpcFailure(error, "vehicle_reference_create");
  return { ok: true, data: record(data) };
}
...
export async function applyVehicleReferenceResolution(
  ...
  if (error) return rpcFailure(error, "vehicle_reference_apply");
  return { ok: true, data: record(data) };
}
...
function parseTaxonomyMappingQueueItem(value: unknown): TaxonomyMappingQueueItem | null {
  ...
  if (!listingId || !status) return null;
