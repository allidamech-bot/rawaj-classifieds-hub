...
export async function safetySaveCase(
  ...
  if (safetyCaseMutationInFlight.has(key))
    return inProgress("حفظ القضية قيد التنفيذ بالفعل.", "admin_safety_case_save_in_progress");
  safetyCaseMutationInFlight.add(key);
  try {
    return fromApi(
      await cloudflareApiRequest<{ id: string; version: number; updatedAt: string }>(
        payload.id
          ? `/v1/admin/safety/cases/${encodeURIComponent(payload.id)}`
          : "/v1/admin/safety/cases",
        { method: payload.id ? "PATCH" : "POST", body: { ...payload, id: undefined } },
      ),
    );
  } finally {
    safetyCaseMutationInFlight.delete(key);
  }
}
export async function safetySetCaseStatus(
  ...
  try {
    return fromApi(
      await cloudflareApiRequest<{ id: string; version: number; updatedAt: string }>(
        `/v1/admin/safety/cases/${encodeURIComponent(payload.id)}/status`,
...
export async function safetyEscalateCase(
  ...
  try {
    return fromApi(
      await cloudflareApiRequest<{ id: string; version: number; updatedAt: string }>(
        `/v1/admin/safety/cases/${encodeURIComponent(payload.id)}/escalate`,
