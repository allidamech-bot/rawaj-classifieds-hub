...
import { escapePostgrestFilterValue, escapePostgrestSearchTerm, getClient, mapError } from "@/lib/api/shared";
...
export async function fetchPublicListingsLocationAware(
  ...
  if (filters.districtAr) {
    const subtreeIds = await resolveLocationSubtreeIds(client, filters.governorateId, filters.districtAr);
    query = subtreeIds.length > 0
...
function applySort(query: any, sort: string) {
  if (sort === "cheapest") return query.order("price", { ascending: true, nullsFirst: false }).order("id", { ascending: true });
  if (sort === "expensive") return query.order("price", { ascending: false, nullsFirst: false }).order("id", { ascending: true });
  if (sort === "featured") return query.order("is_featured", { ascending: false }).order("created_at", { ascending: false }).order("id", { ascending: false });
...
function applyCursor(query: any, cursor: ListingCursor | null) {
  ...
  if (cursor.type === "featured") {
    ...
    return cursor.is_featured
      ? query.or(`is_featured.eq.false,and(is_featured.eq.true,created_at.lt.${created}),and(is_featured.eq.true,created_at.eq.${created},id.lt.${id})`)
      : query.or(`and(is_featured.eq.false,created_at.lt.${created}),and(is_featured.eq.false,created_at.eq.${created},id.lt.${id})`);
  }
  ...
  const op = cursor.type === "cheapest" ? "gt" : "lt";
  return query.or(`price.${op}.${price},price.is.null,and(price.eq.${price},id.gt.${id})`);
}
