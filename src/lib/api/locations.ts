import type { LocationNode, ClassifiedsResult } from "@/lib/classifieds-types";
import {
  getClient,
  mapError,
  rowArray,
...
export async function fetchLocationPath(id: string): Promise<ClassifiedsResult<LocationNode[]>> {
  const clientResult = getClient();
  if (!clientResult.ok) return clientResult;

  const { data, error } = await clientResult.data.rpc("rawaj_location_path", { node_id: id });
  if (error) return { ok: false, error: mapError(error) };

  const nodes = ((data ?? []) as Record<string, unknown>[]).map((row) =>
    mapLocationNode({
      ...row,
      country_code: "SY",
      is_active: true,
