...
export const Route = createFileRoute("/advertise")({
  ...
  head: () =>
    createSeo({
      ...
      description: "اطلب مساحة إعلانية في رواج أو حملة مخصصة، ثم راجع السعر والتجهيز مع فريق رواج قبل التفعيل.",
      path: "/advertise",
      noindex: true,
    }),
...
const requestKinds: Array<{ value: AdvertisingRequestKind; ar: string; en: string; hintAr: string; hintEn: string }> = [
  { value: "home", ar: "الرئيسية", en: "Home", hintAr: "ظهور في المساحة الإعلانية بالصفحة الرئيسية", hintEn: "Placement on the home page" },
  { value: "search_results", ar: "نتائج البحث", en: "Search results", hintAr: "ظهور أثناء تصفح نتائج البحث", hintEn: "Placement within search results" },
  { value: "categories", ar: "الأقسام", en: "Categories", hintAr: "ظهور داخل صفحات الأقسام", hintEn: "Placement on category pages" },
  { value: "campaign", ar: "حملة إعلانية", en: "Ad campaign", hintAr: "حملة مخصصة بأكثر من موضع حسب الاتفاق", hintEn: "A custom multi-placement campaign" },
];
...
function AdvertisePage() {
  ...
  if (auth.status === "loading") {
    return <StatePanel title={text("جارٍ تجهيز طلب الإعلان", "Preparing advertising request")} />;
  }

  if (auth.status !== "signedIn") {
    return (
      <>
        <PageHeader title={text("أعلن على رواج", "Advertise on RAWAJ")} to="/more" backMode="history" />
        <main className="container-wide mobile-page-bottom pt-4">
          ...
          >
            <Link to="/login" className="rawaj-button-primary mt-4 inline-flex min-h-11 items-center px-5">
  ...
  return (
    <>
      <PageHeader title={text("أعلن على رواج", "Advertise on RAWAJ")} to="/more" backMode="history" />
      <main className="rawaj-advertise-page container-wide mobile-page-bottom space-y-5 pt-4" dir={language === "ar" ? "rtl" : "ltr"}>
        <section className="overflow-hidden rounded-2xl border border-amber-400/20 bg-[#17130e] p-5 shadow-soft sm:p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-amber-300/80">RAWAJ ADS</p>
        ...
        {loadError ? (
          <section className="rounded-2xl bg-card p-4 hairline">
            <p role="alert" className="text-xs font-semibold text-destructive">{loadError}</p>
            <button type="button" disabled={loading} onClick={() => void load()} className="rawaj-button-primary mt-3 min-h-10 px-4 text-xs">
        ...
        <form onSubmit={(event) => void submit(event)} aria-busy={saving} className="space-y-5">
          <section className="rounded-2xl bg-card p-4 hairline sm:p-5">
            <StepTitle icon={<Megaphone />} title={text("أين تريد الإعلان؟", "Where do you want to advertise?")} />
            <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4" role="radiogroup" aria-label={text("نوع الطلب الإعلاني", "Advertising request type")}>
              {requestKinds.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="mt-1 block text-[10px] leading-5 text-muted-foreground">{text(item.hintAr, item.hintEn)}</span>
                  </button>
                );
              })}
            </div>
          ...
          <section className="rounded-2xl bg-card p-4 hairline sm:p-5">
            <StepTitle icon={<Store />} title={text("اربط الطلب بإعلانك إن رغبت", "Link one of your listings if useful")} />
            <label className="mt-3 block">
              <span className="mb-1.5 block text-xs font-bold text-muted-foreground">{text("الإعلان المرتبط", "Related listing")}</span>
              <select value={listingId} onChange={(event) => setListingId(event.target.value)} className="input min-h-12 w-full" disabled={loading && !loaded}>
                <option value="">{text("بدون ربط بإعلان — طلب عام", "No listing — general request")}</option>
          ...
          <section className="rounded-2xl bg-card p-4 hairline sm:p-5">
            <StepTitle icon={<CalendarDays />} title={text("المدة والاستهداف", "Duration and targeting")} />
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <Field label={text("المدة المطلوبة", "Requested duration")}>
                <select value={requestedDays} onChange={(event) => setRequestedDays(Number(event.target.value))} className="input min-h-12 w-full">
              ...
              <Field label={text("الأجهزة", "Devices")}>
                <select value={device} onChange={(event) => setDevice(event.target.value as AdvertisingRequestDevice)} className="input min-h-12 w-full">
          ...
          <section className="rounded-2xl bg-card p-4 hairline sm:p-5">
            <StepTitle icon={<MonitorSmartphone />} title={text("تفاصيل الطلب", "Request details")} />
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <Field label={text("رابط الوجهة — اختياري", "Destination URL — optional") }>
                <input value={destinationUrl} onChange={(event) => setDestinationUrl(event.target.value)} type="url" maxLength={500} placeholder="https://..." className="input min-h-12 w-full" />
              </Field>
              <Field label={text("الميزانية التقريبية — اختياري", "Approximate budget — optional") }>
                <input value={budgetNote} onChange={(event) => setBudgetNote(event.target.value)} maxLength={100} placeholder={text("مثال: أريد عرض السعر المتاح", "Example: send me the available quote")} className="input min-h-12 w-full" />
              </Field>
              <Field label={text("ملاحظات أو هدف الحملة — اختياري", "Notes or campaign goal — optional")} wide>
                <textarea value={customerNote} onChange={(event) => setCustomerNote(event.target.value)} maxLength={900} rows={4} className="input w-full resize-y" placeholder={text("اكتب أي تفاصيل تساعد فريق رواج على تجهيز العرض المناسب.", "Add any details that help the RAWAJ team prepare the right proposal.")} />
              </Field>
            </div>
            ...
            <button type="submit" disabled={saving || loading || !loaded} className="mt-4 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-amber-600 px-5 py-3 text-sm font-black text-white disabled:opacity-50 sm:w-auto">
              <Send className="h-4 w-4" />
              {saving ? text("جارٍ إرسال الطلب", "Submitting") : text("إرسال طلب الإعلان", "Submit advertising request")}
            </button>
        ...
        <section className="rounded-2xl bg-card p-4 hairline sm:p-5">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-base font-black">{text("طلباتك الإعلانية", "Your advertising requests")}</h2>
            <span className="rounded-full bg-muted px-3 py-1 text-[10px] font-bold text-foreground">{requests.length}</span>
          </div>
          {loading && !loaded ? (
            <p className="mt-3 text-xs text-muted-foreground">{text("جارٍ التحميل...", "Loading...")}</p>
          ) : requests.length ? (
              ...
              {requests.map((request) => (
                <article key={request.support.id} className="rounded-xl bg-muted-surface p-3 hairline">
                  ...
                  <p className="mt-1 text-[10px] text-muted-foreground">
                    {request.details.requestedDays} {text("يوم", "days")} · {deviceLabel(request.details.device, text)}
                  </p>
                  {request.support.publicResponse ? (
                    <p className="mt-2 rounded-lg bg-card p-2 text-[11px] leading-5 text-foreground hairline">{request.support.publicResponse}</p>
                  ) : null}
                </article>
              ))}
            </div>
          ) : (
            <p className="mt-3 text-xs text-muted-foreground">{text("لا توجد طلبات إعلانية بعد.", "No advertising requests yet.")}</p>
          )}
        </section>
      </main>
    </>
...
function StepTitle({ icon, title }: { icon: React.ReactNode; title: string }) {
  return <h2 className="flex items-center gap-2 text-base font-black"><span className="text-amber-500 [&>svg]:h-4 [&>svg]:w-4">{icon}</span>{title}</h2>;
}

function Field({ label, children, wide = false }: { label: string; children: React.ReactNode; wide?: boolean }) {
  return <label className={wide ? "block sm:col-span-2" : "block"}><span className="mb-1.5 block text-xs font-bold text-muted-foreground">{label}</span>{children}</label>;
}

function StatePanel({ title, body, children }: { title: string; body?: string; children?: React.ReactNode }) {
  return <section className="rounded-2xl bg-card p-7 text-center hairline"><Megaphone className="mx-auto h-7 w-7 text-amber-500" /><h1 className="mt-3 text-base font-black">{title}</h1>{body ? <p className="mx-auto mt-2 max-w-xl text-xs leading-6 text-muted-foreground">{body}</p> : null}{children}</section>;
}
...
function deviceLabel(device: AdvertisingRequestDevice, text: (ar: string, en: string) => string): string {
...
function Status({ status, text }: { status: AdvertisingRequest["support"]["status"]; text: (ar: string, en: string) => string }) {
  const label = status === "new" ? text("جديد", "New") : status === "under_review" ? text("قيد المراجعة", "Under review") : status === "resolved" ? text("تمت المعالجة", "Resolved") : text("مغلق", "Closed");
  return <span data-status={status} className="rawaj-advertise-status rounded-full px-2.5 py-1 text-[10px] font-extrabold">{label}</span>;
}
