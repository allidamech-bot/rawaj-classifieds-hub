...
test("production gateway hosts are explicit and narrowly allowed", () => {
  ...
  assert.equal(
    isAllowedGatewayHost("rawaj-market-gateway-a1b2c3-owner.vercel.app"),
    true,
  );
...
test("decision order honors explicit and stored choices before geography", () => {
  assert.deepEqual(
    resolveMarketDecision({ explicit: "SY", stored: "SA", country: "SA" }),
    {
      market: "SY",
      source: "explicit",
...
test("customer and admin cookies are separate and contain market codes only", () => {
  assert.notEqual(
    preferenceCookieName("customer"),
    preferenceCookieName("admin"),
  );
  ...
  assert.equal(
    parseCookiePreference(
      `${preferenceCookieName("customer")}=bad%ZZ`,
      "customer",
    ),
    null,
  );
...
test("geography suggests a customer market without forcing a redirect", async () => {
  const response = await worker.fetch(
    requestWithCountry("https://gateway.local/", "SA"),
  );
...
test("production hostname controls scope and cannot be overridden by query parameters", async () => {
  const customer = await worker.fetch(
    new Request("https://go.rawa-j.com/?scope=admin"),
  );
  assert.match(await customer.text(), /اختر السوق الذي تريد تصفحه/);

  const admin = await worker.fetch(
    new Request("https://admin.rawa-j.com/?scope=customer"),
  );
  assert.match(await admin.text(), /أي سوق تريد إدارته؟/);

  const customerChoice = await worker.fetch(
    new Request("https://go.rawa-j.com/go/SA?scope=admin"),
  );
  assert.equal(
    customerChoice.headers.get("Location"),
    "https://sa.rawa-j.com/",
  );

  const adminHandoff = await worker.fetch(
    new Request("https://go.rawa-j.com/admin"),
  );
  assert.equal(adminHandoff.status, 302);
  assert.equal(
    adminHandoff.headers.get("Location"),
    GATEWAY_ORIGINS.admin + "/",
  );
});
...
test("explicit and remembered choices redirect while setting only the scoped preference", async () => {
  ...
  assert.doesNotMatch(
    explicit.headers.get("Set-Cookie") ?? "",
    /preferredAdminMarket/,
  );
  ...
  assert.equal(
    remembered.headers.get("Location"),
    "https://sa.rawa-j.com/admin",
  );
...
test("market choice endpoints reject open redirects and set safe response headers", async () => {
  const selected = await worker.fetch(
    new Request(
      "https://gateway.local/go/SA?scope=customer&next=https://evil.example",
    ),
  );
  assert.equal(selected.status, 302);
  assert.equal(selected.headers.get("Location"), "https://sa.rawa-j.com/");
  assert.match(
    selected.headers.get("Content-Security-Policy") ?? "",
  ...
  const unknown = await worker.fetch(
    new Request("https://gateway.local/go/TR"),
  );
  assert.equal(unknown.status, 404);
  const missing = await worker.fetch(
    new Request("https://gateway.local/unknown"),
  );
  ...
  const unrecognizedHost = await worker.fetch(
...
test("health and HEAD requests remain bounded and body-free", async () => {
  const health = await worker.fetch(
    new Request("https://gateway.local/health"),
  );
  ...
  const head = await worker.fetch(
    new Request("https://gateway.local/", { method: "HEAD" }),
  );
...
test("release readiness records verified production evidence and outstanding admin blockers", async () => {
  const [readinessJson, packageJson, vercelConfigJson, wranglerConfig] =
    await Promise.all([
  ...
  for (const key of [
    "admin_vercel_domain_created",
    "admin_destination_auth_verified",
  ]) {
  ...
  assert.equal(
    packageConfig.scripts["release:gate"],
