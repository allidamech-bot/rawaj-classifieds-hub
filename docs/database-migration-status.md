...
| `supabase/migrations/202607020003_owner_admin_moderator_policy_alignment.sql` | live-absent / historical intent | Latest repository policy intent separates moderation from owner-only hard delete, but confirmed Live inspection showed old Admin-like policies remained. Do not replay blindly. Narrow reconciliation was applied separately as `202607050002`. |
...
| `supabase/migrations/202607050001_reconcile_listing_image_storage_visibility.sql` | reconciliation | Removes legacy bucket-wide object read and limits public object access to approved, non-archived listings while keeping the bucket private. Applied and structurally verified on Live. |
| `supabase/migrations/202607050002_reconcile_live_authorization_alignment.sql` | reconciliation | Narrow Live authorization reconciliation replacing old Admin-like policies with owner-only hard delete and Privileged moderators moderation paths. Applied and structurally verified on Live. Does not claim full behavioral authorization matrix testing. |
| `supabase/migrations/202607130001_canonical_listing_taxonomy_assignments.sql` | live-unverified | Canonical listing-to-taxonomy relation, legacy-derived backfill, owner-only explicit leaf assignment RPC, and RLS-safe reads. Repository intent is complete; live application must be proven before relying on the table or RPC. |
