...
const cleanupPath = new URL('../supabase/demo-data/remove_launch_demo_listings.sql', import.meta.url);
...
const categories = [
  'cars', 'realestate', 'mobiles', 'electronics', 'furniture', 'jobs', 'services',
  'fashion', 'food', 'animals', 'education', 'business', 'misc',
];
...
test('cleanup targets only the tagged removable batch', () => {
  ...
  assert.match(cleanup, /coalesce\(\(details #>> '\{_rawaj_seed,removable\}'\)::boolean, false\) = true/);
