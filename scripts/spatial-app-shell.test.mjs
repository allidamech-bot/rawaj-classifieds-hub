...
const [root, navigation, shell, header, brand, offlineNotice, dock, primitives, css] =
  await Promise.all([
    ...
    readFile(new URL("../src/components/shell/BrandLockup.tsx", import.meta.url), "utf8"),
...
test("floating navigation includes location, chats, stable branding, and offline feedback", () => {
  ...
  assert.match(header, /<BrandLockup compact=\{compact\} \/>/);
  assert.match(brand, /width=\{32\}/);
  assert.match(brand, /height=\{32\}/);
  assert.match(brand, /draggable=\{false\}/);
