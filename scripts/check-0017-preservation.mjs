...
const migration0017Path = path.join(migrationsDirectory, "0017_listing_moderation_actions_contract.sql");
...
const countBefore = Number(database.prepare("SELECT count(*) AS c FROM listing_moderation_actions").get().c);
console.log(`Before migration 0017: ${countBefore} row(s) in listing_moderation_actions`);
console.log(`restore record exists: ${database.prepare("SELECT count(*) AS c FROM listing_moderation_actions WHERE action = 'restore'").get().c === 1 ? 'yes' : 'no'}`);

database.exec(await readFile(migration0017Path, "utf8"));

const countAfter = Number(database.prepare("SELECT count(*) AS c FROM listing_moderation_actions").get().c);
console.log(`After migration 0017: ${countAfter} row(s) in listing_moderation_actions`);
console.log(`restore record preserved: ${database.prepare("SELECT count(*) AS c FROM listing_moderation_actions WHERE action = 'restore'").get().c === 1 ? 'yes' : 'no'}`);

const indexExists = database.prepare("SELECT count(*) AS c FROM sqlite_schema WHERE name = 'idx_listing_moderation_actions_listing'").get().c === 1;
...
if (database.prepare("SELECT count(*) AS c FROM listing_moderation_actions WHERE action = 'restore'").get().c !== 1) {
