...
begin
  ...
  if current_setting('rawaj.owner_price_drop_write', true) = 'on' then
    ...
    if (to_jsonb(new) - array['price', 'updated_at', 'search_text_normalized', 'price_new_syp_normalized'])
       is distinct from
       (to_jsonb(old) - array['price', 'updated_at', 'search_text_normalized', 'price_new_syp_normalized'])
    then
  ...
  if current_setting('rawaj.owner_reservation_write', true) = 'on' then
    ...
    if (to_jsonb(new) - array['reserved_at', 'updated_at', 'search_text_normalized', 'price_new_syp_normalized'])
       is distinct from
       (to_jsonb(old) - array['reserved_at', 'updated_at', 'search_text_normalized', 'price_new_syp_normalized'])
    then
