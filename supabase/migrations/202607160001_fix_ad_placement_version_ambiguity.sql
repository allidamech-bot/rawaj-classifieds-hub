-- Fix ambiguous output-column references in owner ad-placement write RPCs.
-- Both functions return an output column named version, so the UPDATE expression
-- must qualify the table column explicitly inside PL/pgSQL.
...
begin
  ...
  if p_id is null then
    ...
    set
      ...
      version = ad_placements.version + 1,
      updated_by = v_actor,
      updated_at = now()
