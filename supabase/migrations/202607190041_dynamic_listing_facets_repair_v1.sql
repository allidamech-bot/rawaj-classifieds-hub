-- RAWAJ Taxonomy, Data & Search Foundation V1: repair dynamic facet aggregation.
-- Replaces the initial function before cutover; no Production data is changed.
...
begin
  ...
  scalar_values as (
    select
      rule_row.field_key,
      coalesce(attribute_row.value_key, attribute_row.value_boolean::text) as value_key,
      candidate_row.id as listing_id
    ...
    where rule_row.field_type in ('single_select', 'reference', 'location', 'boolean')
      and coalesce(attribute_row.value_key, attribute_row.value_boolean::text) is not null
  ),
  multi_values as (
    select
      rule_row.field_key,
      selected_value.value as value_key,
      candidate_row.id as listing_id
    from available_rules rule_row
    join candidate_listings candidate_row on true
    join public.listing_attribute_values attribute_row
      on attribute_row.listing_id = candidate_row.id
     and attribute_row.field_key = rule_row.field_key
    cross join lateral jsonb_array_elements_text(attribute_row.value_json) selected_value(value)
    where rule_row.field_type = 'multi_select'
      and attribute_row.value_json is not null
      and jsonb_typeof(attribute_row.value_json) = 'array'
  ),
  option_counts as (
    select value_row.field_key, value_row.value_key, count(distinct value_row.listing_id) as result_count
    from (
      select * from scalar_values
      union all
      select * from multi_values
    ) value_row
    group by value_row.field_key, value_row.value_key
  ),
  numeric_ranges as (
    select
      rule_row.field_key,
