-- RAWAJ idempotent owner draft creation.
--
-- One add-listing flow receives one client-generated UUID. Repeated requests
-- with that UUID return/update the same draft instead of inserting duplicates.

alter table public.listings
  add column if not exists creation_request_id uuid;

create unique index if not exists listings_owner_creation_request_uidx
  on public.listings (owner_id, creation_request_id)
...
comment on function public.rawaj_create_owner_draft_v2(uuid, jsonb) is
  'Creates or updates exactly one owner draft for one client creation request UUID.';
