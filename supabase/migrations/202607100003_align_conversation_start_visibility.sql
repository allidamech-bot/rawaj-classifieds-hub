-- Prevent new conversations from starting on archived or expired listings.
-- Existing conversations remain untouched; only the start/reuse boundary is aligned.
...
declare
  listing_owner uuid;
  listing_status text;
  listing_archived_at timestamptz;
...
begin
  ...
  select owner_id, status, archived_at, expires_at
  into listing_owner, listing_status, listing_archived_at, listing_expires_at
  ...
  if listing_status <> 'approved'
    or listing_archived_at is not null
    or (listing_expires_at is not null and listing_expires_at <= now())
  then
    raise exception 'Conversations can only be started for available public listings.';
  end if;
...
revoke all on function public.rawaj_start_listing_conversation(uuid) from public;
revoke all on function public.rawaj_start_listing_conversation(uuid) from anon;
...
comment on function public.rawaj_start_listing_conversation(uuid) is
  'Starts or reuses an active conversation only for currently available public listings while preserving blocked-relationship enforcement.';
